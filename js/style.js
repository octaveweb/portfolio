// menu box
{
  ...
  document.body.addEventListener("click", (event) => {
    if (
      !menu_box.contains(event.target) &&
      event.target !== menu_icon &&
      !settings_box.contains(event.target) &&
      event.target !== robot
    ) {
      menu_box.style.right = "-100%";
      settings_box.style.right = "-100%";
...
i1.addEventListener("click", () => {changes(i1, s1);});
i2.addEventListener("click", () => {changes(i2, s2);});
i3.addEventListener("click", () => {changes(i3, s3);});
i4.addEventListener("click", () => {changes(i4, s4);});
i5.addEventListener("click", () => {changes(i5, s5);});
